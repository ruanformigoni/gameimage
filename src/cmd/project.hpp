...
// Operations
enum class Op { SET, DEL, };

// set() {{{
[[nodiscard]] inline std::error<std::string> set(std::string const& name) noexcept
{
  // Open db
  auto db_build = ns_db::ns_build::read();
  qreturn_if(not db_build, "Could not open build database");
  // Check if 'name' is present in db
  auto const& opt_metadata = ns_exception::to_expected([&]{ return db_build->find(name); });
  qreturn_if(not opt_metadata, "Could not find project '{}'"_fmt(name));
  // Set as current
...
// del() {{{
[[nodiscard]] inline std::error<std::string> del(std::string_view str_name) noexcept
{
  // Open build db
  auto db_build = ns_db::ns_build::read();
  qreturn_if(not db_build, "Could not open build database");
  ...
  qreturn_if(it == std::ranges::end(db_build->projects)
    , "Project '{}' not found to delete"_fmt(str_name)
  );
